...
class FEM(nn.Module):
    def __init__(self, channel, dim, div=8):
        ...
        self.conv2 = conv3x3_bn_relu(dim * 2, dim)
    ...
    def forward(self, r, d):
        ...
        cat = self.conv2(torch.cat([add, mul], dim=1))
...
class RBM(nn.Module):
    def __init__(self, channel):
        ...
        self.region_attention = RA(dim=channel)
        self.ra = RA(dim=channel)

    def forward(self, r, d, label=None):
        add = r + d
        mul = r * self.sigmoid(d)
        fuse = self.conv2(torch.cat([add, mul], dim=1))

        d_ra = d * self.ra(d, label)
        fuse_ra = fuse * self.region_attention(fuse, label)
        return fuse_ra + d_ra
...
# channel-wise information module
class CIM(nn.Module):
    def __init__(self, in_planes, ratio=16, kernel_size=3, padding=1):
        ...
        self.conv1 = Conv2d(in_planes, in_planes // ratio, 1, bias=False)
        self.conv2 = Conv2d(in_planes // ratio, in_planes, 1, bias=False)

        self.relu = nn.ReLU(inplace=True)
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        x1 = x * self.sigmoid(self.fc2(self.fc1(x)))
        out = self.sigmoid(self.conv2(self.relu(self.conv1(self.max_pool(x1)))))
        return out
...
class IIBNet(nn.Module):
    def __init__(self, n_classes=41):
        ...
        resnet_raw_model1 = convnext_base(in_22k=True)
        resnet_raw_model2 = convnext_base(in_22k=True)
        ...
        self.low_weight = FEM(filters[0], 64, div=8)
        self.high_weight = Fusion1(filters[3])
        # self.high_weight = FEM(filters[3], 64, div=4)
    ...
    def forward(self, rgb, depth, label):
        ...
        # CIIB
        hweight4 = self.high_weight(rgb4, depth4)
        ...
        # SIIB
        lweight1 = self.low_weight(rgb1, depth1)
