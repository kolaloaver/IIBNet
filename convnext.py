...
class ConvNeXt(nn.Module):
    ...
    Args:
        ...
        layer_scale_init_value (float): Init value for Layer Scale. Default: 1e-6.
        head_init_scale (float): Init scaling value for classifier widths and biases. Default: 1.
    """
    ...
    def forward_features(self, x):
        ...
        for i in range(4):
            ...
            x = self.stages[i](x)
            layers.append(x)
        return layers, self.norm(x.mean([-2, -1])) # global average pooling, (N, C, H, W) -> (N, C)

    def forward(self, x):
        layers, x = self.forward_features(x)
        x = self.head(x)
        return layers
...
def convnext_base(pretrained=False, in_22k=False, **kwargs):
    ...
    if pretrained:
        # weight path
        path = 'convnext_base_22k_1k_384.pth'
        url = path
        model.load_state_dict(torch.load(url, map_location='cpu')['model'], strict=False)
    return model
